...
class QuickScrollList extends React.Component {
	...
	onThumbDrag = (event) => {
		const { data, itemHeight, thumbHeight, viewportHeight, numColumns } = this.props;
		...
		if (gestureY >= 0 && gestureY <= availableHeight) {
			...
			let lastIndex = data.length - Math.floor(viewportHeight / itemHeight) + numColumns;
			let index = Math.floor(lastIndex * thumbPos) / numColumns;
			if (index > lastIndex) index = lastIndex;
			if (index < 0) index = 0;
			Animated.event([{ nativeEvent: { absoluteY: this.position } }], {useNativeDriver:false})(event);
